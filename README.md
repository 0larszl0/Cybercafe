...
Feel free to download the cybercafe.ttc file to use this old-school font!
...
### FontForge
Once all the SVGs had been created I took to fontforge because of its reliability and free-to-use software.
Some initial hiccups first came around when my original svg creation utilised masks and groups, wherein I
found out that fontforge doesn't account for them, and simply ignores the masks. As a result, it's important
for anyone else who wants to import there SVGs to the forge, you must flatten them out. Simply put, you need
all your hole outlines to be in the same path as the main outline of the shape. HOWEVER, those hole coordinates
MUST wind the other way to utilise the "nonzero" fill rule. For me this was simple, just [::-1] my list of tuples
for the holes and then add it to the accumulated path string.

Anyway, instead of manually adding each SVG into their respectable slot, copy the code from the auto-forge.py 
